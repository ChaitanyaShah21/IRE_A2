...
**D42 resubmission scored (2026-09-26):** MIND **0.6319** (rank 63, was 0.6181 / 69), EB-NeRD **0.7621** (rank 74, was 0.7397 / 77; self-hosted worker, disclose per D32). EB-NeRD matches offline to 0.0008; MIND delivers 62% of its offline delta. Full table: `NUMBERS.md` I.6. `main` push still pending from Chaitanya's terminal.
...
|---|---|---|---|---|
| 2026-09-25 | **Found by reading, before running:** re-scoring the leaderboard with the D42 rack model would have silently re-emitted the *shipped* model's scores | `run_submission_lgbm.py` resumes by skipping any user-group whose score file exists, and the cache folder `lgbm_scores_{ds}/` was keyed by dataset only. All 64 groups from the 19 September run were on disk, so a `--model lgbm_{ds}_rack.txt` run would have skipped every group and written the old scores to the new file - a valid, validated, *identical* submission | Cache, feature sample and output file are keyed by a model suffix (`_rack`; the default model keeps `""` and its original paths). The cache also records the model's SHA-256 and refuses to resume a cache filled by different bytes, or an unrecorded one. `tests/test_submission_cache.py`, 5 tests; mutating the hash comparison fails one | The two old unmarked caches now refuse a default-model rerun until deleted - a loud stop, chosen over guessing their provenance. Output name changes to `{ds}_lgbm_rack.txt`, so the validator needs `--suffix _rack` |
| 2026-09-20 | **Every NRMS 'epoch' trained on identical negatives with a fresh optimiser** — found by re-reading the training loop while the word-level run was in flight, not by any failure. Affects D41's reported arms and the 6-epoch re-run | `nrms.train` was written to own its whole epoch loop, so it seeds `rng` and builds `Adam` once per **call**. Epoch selection then forced `run_nrms.py` to drive it one epoch at a time (`epochs=1` in an outer loop), because the best epoch is chosen on a validation score computed between epochs — so the per-call setup silently became per-epoch. Consequence: `training_samples` redrew the **same** negatives in the same order every epoch, contradicting its own docstring ("Re-drawn every epoch"), and Adam's momentum was discarded each epoch. Verified directly: two fresh-seeded calls return byte-identical sample arrays | **Documented and made opt-in, not silently corrected.** `train` gained `epoch_offset` and `optimizer` parameters whose defaults reproduce the old behaviour exactly; passing `optimizer=opt, epoch_offset=ep` trains properly. Two tests pin both halves, so changing the default now fails loudly instead of quietly moving every reported NRMS number | **The ablation conclusions stand**: every D41 arm ran this identical procedure and the arms differ only in their time inputs, so the paired CIs compare like with like. What is depressed is the **absolute** level — and it partly explains the recorded "validation still rising at epoch 3", since the optimiser restarts each epoch. Not fixed for the running word-level job **on purpose**: D43 compares word-level against D41's sentence-level numbers, so changing the training procedure would confound architecture with optimisation — the same confound that ruled out training at a larger scale on Kaggle (D43a). Fixing it properly means re-running every arm |
