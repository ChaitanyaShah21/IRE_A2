...
| `scripts/run_submission_lgbm.py` (model-keyed cache, `claim_cache`), `tests/test_submission_cache.py` | AI-generated; fix chosen by Chaitanya from two options (2026-09-25) | Stops a re-score with a new model silently reusing the old model's cached scores. 5 tests; mutating the hash comparison fails one. **Known gap:** the tests call `claim_cache` directly, so deleting its call site in `main()` would not fail a test - covering that needs a full leaderboard run |
| `reports/design_note_a2_skeleton.docx` | AI-generated **notes only**, 2026-09-26 | Section headings, numbers from `NUMBERS.md` as note fragments, and unsupported-claim warnings, around Chaitanya's own paragraph 1 (verbatim). Chaitanya asked for a full draft in his voice; declined under the TA's authorship condition. The report prose is his |
| `reports/IRE_Assignment_2.docx`, `reports/IRE_Assignment_2_report.pdf` | **Human-written** (Chaitanya), 2026-09-26 | The submitted A2 report, 5 pages. Prose is his, written from the skeleton's notes; the data tables were carried over from the skeleton (figures from `NUMBERS.md`). Claude checked every claim against `NUMBERS.md`; one error found and fixed by him (warm p99 30.53 -> 48.93 ms). He chose to leave an AI-usage line out of the report; this log is the disclosure |
...
| `reports/design_note_a2_ai_draft.{md,pdf}` | **AI-written** — renamed 2026-09-21 | Was `design_note_a2.{md,pdf}` and was submitted on 20 September. Kept under an explicit `_ai_draft` name for the Q7.4 audit trail after the TA required the report to be the student's own work. Every figure in it traces to `NUMBERS.md`, but it is not Chaitanya's writing and is not offered as such. **`main` still carries it under the original name**, which is the submitted state |
| `reports/design_note_a2.{md,pdf}` | **to be human-written** | Chaitanya writes it from `reports/report_prep.md`. Does not exist on this branch until he does — the absence is deliberate, so the gap is visible rather than papered over by the AI draft sitting at the expected path |
| `scripts/run_retrieved_regime.py` (`--models base rack`) | AI-generated, **AI-decided** | Scores several models over the *same* retrieved candidates, which is what made D42d's artefact visible |
| `scripts/run_nrms.py` (`--val-impressions`), `src/newsrec/rank/nrms.py` (`epoch_offset`, `optimizer`, flushed progress) | AI-generated | Defaults reproduce the previously reported behaviour exactly; the opt-in path and the two tests exist so the 2026-09-20 epoch defect cannot be changed silently |
| `reports/nrms_ablation_mind_test_word.csv`, `retrieved_regime_mind_test_k100_d42c.csv` | Generated output | From B7 and `run_retrieved_regime.py --models base rack` |
